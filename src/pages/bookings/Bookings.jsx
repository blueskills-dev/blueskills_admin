    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          'https://blueskills3-latest.onrender.com/booking/',
          {
            method: 'GET',
            headers: {
    ...
    const dS =
      dataSource && dataSource.length > 0
        ? dataSource.map((row, idx) => {
            return {
              id: idx + 1,
              title: <div className="">{row['title']}</div>,
              description: <div>{row['description']}</div>,
              address: <div className="">{row['address']}</div>,
              phone: <div className="">{row['phone']}</div>,
              status: <div className="">{row['status']}</div>,
              details: (
                <Link to={`/booking-details/${row.id}`}>
      ...
      <Content>
        ...
        <PageBody>
          <MainWrapper>
            ...
            <ScollableFlex>
              <QuaterWidget title="Total Funding" subtitle="N2,000,000" />
              <QuaterWidget title="Total Expenses" subtitle="N2,000,000" />
              <QuaterWidget title="Total Yield" subtitle="N2,000,000" />
            </ScollableFlex>
